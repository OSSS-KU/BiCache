...
class RotaryEmbedding(nn.Module):
    ...
    def forward(self, q: torch.Tensor, k: torch.Tensor, prefix_len: int, suffix_cache_length: int) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        with torch.autocast(q.device.type, enabled=False):
            ...
            prefix_pos_sin = pos_sin[:, :, :prefix_len, :]
            prefix_pos_cos = pos_cos[:, :, :prefix_len, :]

            block_len = query_len - prefix_len
            block_pos_sin = pos_sin[:, :, key_len - block_len - suffix_cache_length : key_len - suffix_cache_length, :]
            block_pos_cos = pos_cos[:, :, key_len - block_len - suffix_cache_length : key_len - suffix_cache_length, :]

            q_ = self.apply_rotary_pos_emb(
                torch.cat((prefix_pos_sin, block_pos_sin), dim=-2),
                torch.cat((prefix_pos_cos, block_pos_cos), dim=-2),
                q_,
            )
...
class LLaDABlock(nn.Module):
    ...
    def attention(
        ...
        dual_kv: Optional[Tuple[Tuple[torch.Tensor, torch.Tensor], Tuple[torch.Tensor, torch.Tensor]]] = None,
        ...
        if dual_kv is not None:
            (prefix_key, prefix_value), (suffix_key, suffix_value) = dual_kv
            sliced_k = [k[:, :, :prefix_len, :], prefix_key, k[:, :, prefix_len:, :], suffix_key]
            sliced_v = [v[:, :, :prefix_len, :], prefix_value, v[:, :, prefix_len:, :], suffix_value]

        if prefix_kv:
            prefix_key, prefix_value = prefix_kv
            sliced_k = [prefix_key] + sliced_k
            sliced_v = [prefix_value] + sliced_v

        k = torch.cat(sliced_k, dim=-2)
        v = torch.cat(sliced_v, dim=-2)

        present = (k, v) if use_cache else None
        new_kv = (k, v)
        query_len, key_len = q.shape[-2], k.shape[-2]  # could be different if layer_past not None
        suffix_cache_len = dual_kv[1][0].shape[-2] if dual_kv is not None else 0

        if self.config.rope:
            # Apply rotary embeddings.
            q, k = self.rotary_emb(
                q, k, 
                prefix_len if prefix_len else 0, 
                suffix_cache_len
            )
        ...
        return self.attn_out(att), present, new_kv
...
class LLaDALlamaBlock(LLaDABlock):
    ...
    def forward(
        ...
        if self._activation_checkpoint_fn is not None:
            ...
            att, cache, new_kv = self.attention(
                q, k, v, 
                attention_bias, 
                layer_past=layer_past, 
                use_cache=use_cache,
                prefix_len=prefix_len if prefix_kv is None else 0,
                prefix_kv=prefix_kv,
                dual_kv=dual_kv,
            )
...
class LLaDAOutput(NamedTuple):
    ...
    """
    new_kvo: Optional[Sequence[Tuple[torch.Tensor, torch.Tensor, torch.Tensor]]]
...
class LLaDAModel(nn.Module):
    ...
    def forward(
        ...
        prefix_len: Optional[int] = None,
        prefix_cache: Optional[Sequence[Tuple[torch.Tensor, torch.Tensor]]] = None,
        prefix_hidden_state: Optional[torch.Tensor] = None,
        dual_cache: Optional[Tuple[Tuple[torch.Tensor, torch.Tensor], Tuple[torch.Tensor, torch.Tensor]]] = None,
    ) -> LLaDAOutput:
        """
        ...
        attn_key_values: Optional[List[Tuple[torch.Tensor, torch.Tensor]]] = [] if use_cache else None
        new_kvo: Optional[List[Tuple[torch.Tensor, torch.Tensor, Optional[torch.Tensor]]]] = []  # k, v, out tensor of each layer
        ...
        if self.config.block_group_size == 1:
            for block_idx, block in enumerate(self.transformer.blocks):
                ...
                prefix_kv = None if prefix_cache is None or block_idx >= len(prefix_cache) else prefix_cache[block_idx]
                dual_kv = None if dual_cache is None else (dual_cache[0][block_idx], dual_cache[1][block_idx])
                if (
                    (self.activation_checkpointing_strategy == ActivationCheckpointingStrategy.whole_layer)
                    or (
                        self.activation_checkpointing_strategy == ActivationCheckpointingStrategy.one_in_two
                ...
                if attn_key_values is not None:
                    assert cache is not None
                    attn_key_values.append(cache)
                if new_kvo is not None:
                    new_o = x.detach().clone() if prefix_kv is None else None
                    new_kvo.append((*new_kv, new_o))
                if prefix_kv and block_idx == len(prefix_cache) - 1:
                    x = torch.cat((prefix_hidden_state, x), dim=1)
        else:
...
class LLaDAModelLM(PreTrainedModel):
    ...
    def forward(
        ...
        prefix_len: Optional[int] = None,
        prefix_cache: Optional[Sequence[Tuple[torch.Tensor, torch.Tensor]]] = None,  # [(k1, v1), (k2, v2), ...]
        prefix_hidden_state: Optional[torch.Tensor] = None,  # to generate q for non-caching layer
        dual_cache: Optional[Tuple[Sequence[Tuple[torch.Tensor, torch.Tensor]], Sequence[Tuple[torch.Tensor, torch.Tensor]]]] = None,  # (prefix, suffix) **not for shared prefix**
        ...
        if prefix_cache and prefix_hidden_state is None:
            raise ValueError("prefix_hidden_state is required if non-caching layers exist.")
        
        if (prefix_cache or dual_cache) and prefix_len is None:
            raise ValueError("prefix_len is required for either prefix or dual caching")
        
        if prefix_len is not None and prefix_cache is not None:
            assert prefix_len == prefix_cache[0][0].shape[-2]
